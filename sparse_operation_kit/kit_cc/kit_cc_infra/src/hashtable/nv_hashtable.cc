...
template <typename KeyType, typename ValType>
std::shared_ptr<NvHashTable<KeyType, ValType>> 
NvHashTable<KeyType, ValType>::create(size_t capacity, size_t count) {
    return std::shared_ptr<NvHashTable<KeyType, ValType>>(new NvHashTable<KeyType, ValType>(capacity, count));
}

template <typename KeyType, typename ValType>
size_t NvHashTable<KeyType, ValType>::get_and_add_value_head(size_t counter_add, cudaStream_t stream) {
    return hashtable_.get_and_add_value_head(counter_add, stream);
}

template <typename KeyType, typename ValType>
void NvHashTable<KeyType, ValType>::get(const void *d_keys, void *d_vals, size_t len, cudaStream_t stream) const {
...
template <typename KeyType, typename ValType>
void NvHashTable<KeyType, ValType>::insert(const void *d_keys, const void *d_vals, size_t len, cudaStream_t stream) {
    const KeyType *_d_keys = reinterpret_cast<const KeyType*>(d_keys);
...
template <typename KeyType, typename ValType>
void NvHashTable<KeyType, ValType>::dump(void* d_key, void* d_val, size_t* d_dump_counter, cudaStream_t stream) const {
