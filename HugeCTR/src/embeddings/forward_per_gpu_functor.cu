...
/**
 * 0x02 总述
反向传播是求各种权重的变化对最终的误差能造成什么样的影响，或者说是各种权重怎么调整能让预估误差尽可能小，
 其实就是给各种权重找到梯度下降最快的方向，让损失函数快速地全局达到一个最优点。
...
假如batch_size=2，slot_num=2，给出一个CSR例子格式如下（两个样本）：
    ...
    * value: 40,50,10,20,30,50,10,30,20,10
6.1.1 前向传播
下图是前向传播的embedding look示例，最后生成的 embedding_feature 之中，embedding vector个数是：batch_size x slot_num，
 针对我们的例子：40,50,10,20,30,50,10,30,20,10，分成slot就是：[40,50,10,20],[30,50,10],[30,20],[10]。分别对应embedding_feature矩阵中的四行。

注：最后输出的是 train_output_tensors_，中间变量为 embedding_feature，
 embedding_feature 经过了几次GPU之间的通信变化之后演化成了train_output_tensors_ ，
 两者维度相同，所以我们就使用 embedding_feature。下面图之中数字是构造出来，只供演示使用。
007-002.jpg
我们给出 embedding_feature 之中第三条向量的计算过程，他对应了第二个样本的第一个slot，就是 "30,20"。
 所以就是从 hash_table_value 选出了第2行，第3行，对应位置元素相加，即图中给出的计算过程。
007-003.jpg
6.1.2 后向传播
我们再考虑后向传播。
007-004.jpg
后向传播时候用梯度来更新权重，g31,g32,g33,g34 这一行就应该更新 hash_table_value 的第2行，第3行。
 另外，如果假设第二个样本的第一个slot 是 "30,20,20,20"，那么其实就应该用梯度更新hash_table_value 的第2行三次，
 第3行一次。其实也可以看出来，这种更新不要知道 train_value的数值究竟是什么。
...
我们先用常规思路来梳理一下上面例子：
      ...
      嵌入层输出是embedding_feature。
              ...
              其中第三条向量对应了第二个样本的第一个slot，就是 "30,20"。所以就是从 hash_table_value 选出了第2行，第3行，
              对应位置元素相加： 10,220,330,440,550 = (10+100),(20+200),(30+300),(40+400),(50+500)。

      如果有了梯度稠密向量，其是被 hash table value 若干稠密向量做pooling得到的结果。
            比如，梯度矩阵第三条向量 g31,g32,g33,g34 对应的就是 embedding_feature 第三条向量 10,220,330,440,550，
            如果梯度更新权重，就应该更新hash_table_value 的第2行，第3行。
...
我们接着从CUDA角度来看如何更新，其目的是让每一个block 更新一个低维矩阵 hash_table_value 的一行，所以有几个问题：
    ...
    更新这几次，分别用哪一个梯度来更新。
        比如第1个梯度可能更新第二行，第三个梯度也可能更新第二行。针对我们的例子：40,50,10,20,30,50,10,30,20,10，
         分成slot就是：[40,50,10,20],[30,50,10],[30,20],[10]。
         分别对应梯度矩阵中的四行，所以需要从梯度矩阵之中1，2，4行的梯度来更新 10 对应的 hash_table_value。

        具体参见下图，这里 train_value 到 gradient 只是示意，就是逻辑上一一对应。
007-004

这里有一个疑问，为什么不像前向传播那样操作，而是要另外重起炉灶呢？
 这是因为我们不需要知道样本数值就可以更新权重，不需要把40,50,10,20,.....,等等重新走一遍操作哈希表的流程。
 所以，接下来就看看HugeCTR如何解决这几个问题，这里代码比较烧脑。
